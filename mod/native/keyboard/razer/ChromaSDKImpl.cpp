...
void CChromaSDKImpl::DisableLights()
{
    if (CreateKeyboardEffect)
    {
...
void CChromaSDKImpl::DisplayStaticColor(int color)
{
    if (CreateKeyboardEffect)
    {
...
void CChromaSDKImpl::DisplayStaticSpecialKeys(int color)
{
    if (CreateKeyboardEffect)
    {
        Keyboard::CUSTOM_KEY_EFFECT_TYPE Effect = {};
        COLORREF Color = RGB((color >> 16) & 255, (color >> 8) & 255, color & 255);

        Effect.Key[HIBYTE(RZKEY_W)][LOBYTE(RZKEY_W)] = Color;
        Effect.Key[HIBYTE(RZKEY_A)][LOBYTE(RZKEY_A)] = Color;
        Effect.Key[HIBYTE(RZKEY_S)][LOBYTE(RZKEY_S)] = Color;
        Effect.Key[HIBYTE(RZKEY_D)][LOBYTE(RZKEY_D)] = Color;

        CreateKeyboardEffect(Keyboard::CHROMA_CUSTOM_KEY, &Effect, NULL);
    }
}

void CChromaSDKImpl::DisplayDamageFlash(JNIEnv* env)
{
    if(CreateKeyboardEffect)
    {
        RZRESULT Result = RZRESULT_INVALID;
        ...
        CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_NONE, NULL, NULL);
        Sleep(500);
        RZRESULT res1 = CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_CUSTOM, &Effect, NULL);
        Sleep(50);
        RZRESULT res2 = CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_NONE, NULL, NULL);

        if (res1 != RZRESULT_SUCCESS) {
            throwException(env, EXCEPTION, "res1 is " + res1);
        }
        if (res2 != RZRESULT_SUCCESS) {
            throwException(env, EXCEPTION, "res2 is " + res2);
        }
        // return res1 == RZRESULT_SUCCESS && res2 == RZRESULT_SUCCESS;
    }
    // return false;
}

void CChromaSDKImpl::UpdateHealthAndArmor(float health, float armor)
{
    if (CreateKeyboardEffect)
    {
        ...
        // Function keys as armor
        UINT RemainingArmor = UINT(armor * 12.0f); // 12 function keys

        UINT ArmorLevel = ChromaSDK::Keyboard::RZKEY_F1;
        ...
        // Number keys as health
        UINT RemainingHealth = UINT(health * 10.0f); // 10 number keys

        COLORREF HealthColor = RGB((((10-RemainingHealth)/10.0)*255), ((RemainingHealth/10.0)*255), 0);

        UINT HealthLevel = ChromaSDK::Keyboard::RZKEY_1;
